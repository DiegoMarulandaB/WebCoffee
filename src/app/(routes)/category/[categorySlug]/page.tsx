  ...
  return (
    <div className="max-w-6xl py-4 mx-auto sm:py-16 sm:px-24">
      {result && !loading && result.length > 0 && (
        <h1 className="text-3xl font-bold">Café {result[0].attributes.category.data.attributes.categoryName}</h1>
      )}
      ...
      <div className="sm:flex sm:justify-between">
        ...
        <div className="grid gap-5 mt-8 sm:grid-cols-2 md:grid-cols-2 md:gap-10">
          ...
          {filteredProducts && !loading
            ? filteredProducts.map((product: ProductType) => <ProductCard key={product.id} product={product} />)
            : !loading && <p>Lo sentimos, no hemos encontrado productos que coincidan con tu búsqueda.</p>}
        </div>
      </div>
    </div>
  );
